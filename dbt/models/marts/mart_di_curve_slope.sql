-- Daily shape of the DI curve: one row per trade date, built entirely from
-- `mart_di_curve_points` (a mart-on-mart `ref`, not a second pass over the strip —
-- the grid points are defined in exactly one place).
--
...
SELECT
    ...
    CASE
        ...
        WHEN rate_10y_pct - rate_2y_pct >  {{ flat_threshold_pp }} THEN 'normal'
        WHEN rate_10y_pct - rate_2y_pct < -{{ flat_threshold_pp }} THEN 'inverted'
        ELSE 'flat'
    END AS curve_shape
FROM pivoted
