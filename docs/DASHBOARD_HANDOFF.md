...
| **mart_di_curve_points** | tenor × date | `trade_date, tenor_label, target_tenor_years, curve_point_key, lower_symbol, upper_symbol, lower_tenor_years, upper_tenor_years, lower_rate_pct, upper_rate_pct, di_rate_pct, max_leg_gap_years, both_legs_have_open_interest` | The DI1 strip resampled onto a fixed tenor grid (`6m/1y/2y/3y/5y/10y`) by bracketed linear interpolation — **comparable across dates** (contract symbols roll; grid tenors don't). Never extrapolated: a tenor the strip doesn't straddle has no row. |
| **mart_di_curve_slope** | date | `trade_date, rate_6m_pct … rate_10y_pct, n_curve_points, slope_2s10s_pp, slope_1s5s_pp, slope_6m2y_pp, curve_shape` | One row per day: grid rates pivoted wide, standard slopes, and a `normal/flat/inverted` label (±10 bp dead band on the 2s10s). The "is the curve inverted?" tile. |
